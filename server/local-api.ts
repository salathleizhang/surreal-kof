...
function runMuleRun(args: string[], options: { cwd?: string } = {}): Promise<MuleRunResult> {
  return new Promise<MuleRunResult>((resolve) => {
...
async function runStudio(endpoint: string, body: JsonObject) {
  if (!/^[-\w./]+$/.test(endpoint)) {
...
function normalizeMessages(messages: unknown): string | null {
...
async function runChat(body: JsonObject) {
...
async function saveDataUriPhoto(dataUri: string): Promise<string> {
