// Shared MuleRun CLI wrappers used by both the HTTP routes (local-api.ts) and
// the character-generation pipeline (character-pipeline.ts).
//
...
type StudioBody = Record<string, unknown> & {
  noWait?: boolean;
  pollInterval?: number;
  maxWait?: number;
  site?: string;
...
function addParam(args: string[], key: string, value: unknown): void {
...
export function runMule(args: string[], opts: SpawnOptionsWithoutStdio = {}): Promise<MuleResult> {
  return new Promise<MuleResult>((resolve) => {
...
export async function runStudio(endpoint: string, body: StudioBody = {}) {
  if (!/^[-\w./]+$/.test(endpoint)) {
...
function cleanChatOutput(raw: string): string {
