...
function mergeSkill(base: SkillDefinition, override: Partial<SkillDefinition> = {}): SkillDefinition {
  return {
    ...base,
    ...override,
...
export function createGeneratedCombatDefinition(
  entry: Partial<GeneratedCharacterEntry> = {},
): CombatDefinition {
  const moves = entry.moves || {};
  const custom = entry.combat || {};
  const bodyWidth = custom.body?.width ?? BODY_WIDTH;
  const bodyHeight = custom.body?.height ?? BODY_HEIGHT;
  const defaults: Record<string, SkillDefinition> = {
    attack1: {
      id: 'attack1',
      input: 'attack',
      priority: 10,
