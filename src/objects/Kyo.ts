...
import { CHARACTER_SCALE } from '../config/combat.ts';
import { KYO_COMBAT } from '../characters/kyo/combat.ts';
...
export default class Kyo extends Fighter {
  constructor(scene: any, info: Omit<FighterCreateInfo, 'texturePrefix' | 'combat'>) {
  ...
  init_animations(): void {
    ...
    const frameCounts = this.scene.registry.get('kyoFrameCounts') as number[];
