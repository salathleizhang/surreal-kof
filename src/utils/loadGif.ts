...
export function loadGif(url: string): Promise<DecodedGif> {
  return new Promise<DecodedGif>((resolve, reject) => {
    const gif = GIF() as DecodedGif;
...
export async function registerGifTextures(scene: any, prefix: string, url: string): Promise<number> {
