...
import CollisionWorld from '../src/combat/CollisionWorld.js';
import SkillRunner from '../src/combat/SkillRunner.js';
import { intersectsAabb, localBoxToWorld } from '../src/combat/collision/geometry.js';
import { createGeneratedCombatDefinition } from '../src/characters/generated/createCombatDefinition.js';
...
test('collision world resolves a skill-owned hitbox against target hurtboxes', () => {
  ...
  const effects = { emit() {} };
  const world = new CollisionWorld(scene, effects);
  const owner = {
  ...
  world.registerFighter(owner);
  world.registerFighter(target);

  const count = world.activateHitbox(owner, {
...
test('skill runner fires multiple same-frame events exactly once', () => {
  ...
  const runner = new SkillRunner(fighter, {
    combo: {
